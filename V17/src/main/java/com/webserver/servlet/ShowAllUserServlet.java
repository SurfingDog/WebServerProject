...
//        System.out.println("ShowAllUserServlet:开始生成动态页面...");
        ...
        //user.dat文件用户读取后以User对象形式存入list集合
        try (
...
//            e.printStackTrace();
            logger.error(e.getMessage(),e);//error()记录所有信息
        }
        for (User user : list) {
        ...
        //2.1所有在页面上显示的动态数据都需要存入一个名为Context的类中
        Context context = new Context();
        /*
            向Context中添加要在页面中显示的数，使用类似于Map的put方法。
        ...
        /*
            String process(String temple,Context context)
            该方法就是thymeleaf将模板页面与context中的数据进行结合来生成一个动态页面，返回值就是生成
            后的动态页面的源代码(html代码)
         */
